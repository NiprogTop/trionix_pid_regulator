...
class PidRegulator:
    ...
    def __init__(self):
        ...
        self.pids_ = {
            'roll': PID('roll', enabled=False),
            'pitch': PID('pitch', enabled=True),
            'heading': PID('heading', enabled=True),
    ...
    def handle_switch(self, req):
        ...
        rospy.loginfo(str(self.pids_['depth'].ns_) + "/setpoint" + " --> " + str(self.pids_['depth'].setpoint_))
        return resp
    ...
    def update(self, _):
        ...
        if self.enabled_:
            ...
            cmd.force.x = self.clamp(self.command_.linear.x)
            cmd.force.y = self.clamp(self.command_.linear.y)
            cmd.force.z = self.clamp(self.command_.linear.z)
            cmd.torque.x = self.clamp(self.command_.angular.x)
            cmd.torque.y = self.clamp(self.command_.angular.y)
            cmd.torque.z = self.clamp(self.command_.angular.z)
